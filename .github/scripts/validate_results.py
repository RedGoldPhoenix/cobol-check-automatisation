...
def create_github_comment(programs_data, totals, results_dir):
    ...
    comment = """# 🧪 COBOL Check Test Results
...
""".format(
    ...
    for program, data in programs_data.items():
        passed_icon = "✅" if data['passed'] == data['total_tests'] else "⚠️"
...
def post_github_comment(comment, context):
    ...
    if not context['pr_number']:
        print("ℹ️  No PR number found, skipping GitHub comment")
        return True
    ...
    try:
        ...
        print("⚠️  GitHub CLI (gh) not found, skipping comment")
        return False
...
def validate_coverage_threshold(totals, threshold=80):
    ...
    if coverage >= threshold:
        ...
        print(f"❌ Coverage below threshold by {threshold - coverage}%")
        return False
...
def validate_quality_threshold(totals, threshold=80):
    ...
    if quality >= threshold:
        ...
        print(f"❌ Quality below threshold by {threshold - quality}%")
        return False
...
def validate_all_tests_passed(totals):
    ...
    if totals['total_failed'] == 0:
        ...
        print(f"\n⚠️  {totals['total_failed']} test(s) failed")
...
def check_coverage_trend(results_dir):
    ...
    try:
        ...
        if len(history) < 2:
            print("\nℹ️  Not enough historical data for trend analysis")
            return None
        ...
        if coverage_change < -5:
            print("⚠️  WARNING: Coverage dropped significantly!")
            return False
        elif coverage_change > 0:
            print("✅ Coverage improved!")
            return True
        else:
            print("➡️  Coverage stable")
            return True

    except Exception as e:
        print(f"❌ Error reading metrics history: {e}")
        return None
...
def main():
    ...
    print("="*50)
    print(f"Coverage Check: {'✅ PASS' if coverage_ok else '❌ FAIL'}")
    print(f"Quality Check: {'✅ PASS' if quality_ok else '❌ FAIL'}")
    print(f"Tests Check: {'✅ PASS' if tests_ok else '❌ FAIL'}")
    print("="*50)
